...
* Stumbling blocks:
  - don’t just use `-c bioconda` (may work, but also need conda-forge in the general case)
  - old configuration left in `~/.condarc` sometimes gets in the way
...
* .green[Mamba]: A faster, drop-in replacement for `conda` (written in C++)
