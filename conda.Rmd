...
```
* Bioconda builds upon `conda-forge`

--

## Pitfalls

* Ensure `r` isn’t listed in your channels list in `~/.condarc`
* Don’t just use `-c bioconda`
...
Or run `module load conda` and `source conda_init.sh` everytime you want to use
`conda`.
...
# Highly recommended: [Mamba](https://mamba.readthedocs.io/en/latest/)

* A replacement for `conda` with a much faster dependency solver
* Same command-line interface
* Install from `conda-forge`:
```bash
conda install -n base mamba
```
* Mamba is .green[already installed on Rackham]
...
* [Mambaforge](https://github.com/conda-forge/miniforge):
  Alternative to miniconda with pre-installed Mamba and activated conda-forge
...
Anything installed by Conda is installed into a **Conda environment**.

Each Conda environment contains a set of packages.

An environment named `base` exists by default.

It contains `conda` itself and its dependencies, such as Python.

To use the software in an environment, it needs to be **activated**.

Environments are .green[independent]:

- Environment *a* can have version *x* of a tool, while environment *b* has
  the same tool in version *y*.
- Environments


The `base` environment is the only one that cannot easily be deleted and re-created, so keep it clean:
It should only contain Conda and Mamba (and related)
...
* conda-forge packages are also (often) available for Windows
