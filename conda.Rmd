...
* Conda installs a package and all required dependencies, observing constraints on the version.
    ...
    *Example:* a workflow requires `snakemake` whose most recent version requires
    `python>=3.7`, but another tool the workflow depends on is only compatible with
    Python 3.6, so the dependency resolver needs to find an older `snakemake` version
...
--
* For channels with many packages (conda-forge), for packages with many dependencies,
...
* [Mamba](https://mamba.readthedocs.io/en/latest/) is a faster replacement for `conda`
  - partially written in C++
  - with a much faster dependency solver
...
* The base environment is the only one that cannot easily be deleted and re-created, so keep it clean
  - Avoid installing anything not Conda-related into it (only `mamba`, `conda-build` etc.)
...
* Example: Create an environment mapped `mapping` and install Samtools and BWA into it:
```bash
mamba create -n mapping samtools bwa
```
...
* Or to install into the currently active environment:
```bash
...
* Environments are independent (can have different versions of the same software)
